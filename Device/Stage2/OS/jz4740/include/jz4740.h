...
#define __uart_enable()			( REG8(UART0_FCR) |= UART_FCR_UUE | UART_FCR_FE )
#define __uart_disable()		( REG8(UART0_FCR) = ~UART_FCR_UUE )

#define __uart_enable_transmit_irq()	( REG8(UART0_IER) |= UART_IER_TIE )
#define __uart_disable_transmit_irq()	( REG8(UART0_IER) &= ~UART_IER_TIE )

#define __uart_enable_receive_irq() \
  ( REG8(UART0_IER) |= UART_IER_RIE | UART_IER_RLIE | UART_IER_RTIE )
#define __uart_disable_receive_irq() \
  ( REG8(UART0_IER) &= ~(UART_IER_RIE | UART_IER_RLIE | UART_IER_RTIE) )

#define __uart_enable_loopback()  	( REG8(UART0_MCR) |= UART_MCR_LOOP )
#define __uart_disable_loopback() 	( REG8(UART0_MCR) &= ~UART_MCR_LOOP )

#define __uart_set_8n1()		( REG8(UART0_LCR) = UART_LCR_WLEN_8 )

#define __uart_set_baud(devclk, baud)				\
  do {								\
	REG8(UART0_LCR) |= UART_LCR_DLAB;			\
	REG8(UART0_DLLR) = (devclk / 16 / baud) & 0xff;		\
	REG8(UART0_DLHR) = ((devclk / 16 / baud) >> 8) & 0xff;	\
	REG8(UART0_LCR) &= ~UART_LCR_DLAB;			\
  } while (0)

#define __uart_parity_error()		( (REG8(UART0_LSR) & UART_LSR_PER) != 0 )
#define __uart_clear_errors() \
  ( REG8(UART0_LSR) &= ~(UART_LSR_ORER | UART_LSR_BRK | UART_LSR_FER | UART_LSR_PER | UART_LSR_RFER) )

#define __uart_transmit_fifo_empty()	( (REG8(UART0_LSR) & UART_LSR_TDRQ) != 0 )
#define __uart_transmit_end()		( (REG8(UART0_LSR) & UART_LSR_TEMT) != 0 )
#define __uart_transmit_char(ch)	( REG8(UART0_TDR) = (ch) )
#define __uart_receive_fifo_full()	( (REG8(UART0_LSR) & UART_LSR_DR) != 0 )
#define __uart_receive_ready()		( (REG8(UART0_LSR) & UART_LSR_DR) != 0 )
